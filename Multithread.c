...
static void memory_pool_reset(MemoryPool *pool);
...
#if defined(_WIN32) || defined(_WIN64)
static int thread_create(THREAD_HANDLE *handle, THREAD_FUNC func, void *arg) {
    *handle = CreateThread(NULL, 0, func, arg, 0, NULL);
    return *handle != NULL ? 0 : -1;
}

static int thread_join(THREAD_HANDLE handle) {
...
static int scale_thread_pool(AdaptiveThreadPool *pool, int delta) {
    ...
    if (delta > 0) {
        ...
        for (int i = 0; i < delta && new_count < pool->hard_limit; i++) {
            ...
            if (thread_create(&worker->handle, worker_thread_func, worker) != 0) {
                free(worker);
                break;
            }
...
static int AdaptiveThreadPool_init(AdaptiveThreadPool *self, PyObject *args, PyObject *kwds) {
    ...
    if (thread_create(&self->monitor_thread, monitor_thread_func, self) != 0) {
...
static void AdaptiveThreadPool_dealloc(AdaptiveThreadPool *self) {
    ...
    if (self->monitor_thread) {
        thread_join(self->monitor_thread);
    }
    ...
    for (size_t i = 0; i < self->hard_limit; i++) {
        if (self->workers[i]) {
            thread_join(self->workers[i]->handle);
            free(self->workers[i]);
        }
    }
...
static PyTypeObject AdaptiveThreadPoolType = {
    ...
    .tp_name = "adaptive_threadpool.AdaptiveThreadPool",
...
static PyModuleDef adaptive_threadpool_module = {
    ...
    .m_name = "adaptive_threadpool",
    .m_doc = "Cross-platform adaptive thread pool for Python 3.14 free-threading",
    .m_size = -1,
};

PyMODINIT_FUNC PyInit_adaptive_threadpool(void) {
    PyObject *module;
